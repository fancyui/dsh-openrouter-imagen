...
**这条规则写在工具说明里,不写在自带的 skill 正文里** —— 这个区别是实测出来的,不是设计洁癖。

用 `/xxx` 触发另一个 skill(例如第三方的 `gpt-image-2-style-library`)时,它的产物是**一条成品提示词**。那次实测:对话模型加载了那个 skill、读了它的参考库、产出了提示词,然后直接调 `openrouter_generate_imagen` —— **全程没有加载本插件自带的那个 skill**。技能表只是「目录」,正文要模型自己决定去加载;工具说明却是随**每次**请求一起发的。

所以哪条规则必须生效,就只能放哪一层:

| 层 | 大小 | 何时进上下文 | 能保证什么 |
| --- | --- | --- | --- |
| 工具说明 | 2,987 字符 | **每次请求**(与调用与否无关) | 调用纪律 |
| skill 的目录描述行 | 488 字符 | 技能表里始终有,`skills: false` 时消失 | 决定要不要加载 |
| skill 正文 | 13,810 字符 | 只有模型主动加载那一次 | 方法论、示例、排错 |

因此工具说明里加了一段:**上下文里已经有另一条写好的提示词时,不要自己再写一条,把那段原文整段填进 `prompt`** —— 不要精简、不要换词。防的是重写:模板里的逐字文字、版式层级、负向约束一改就没了。同时明确两件事:

- **它不等于交付。** 「给出提示词」不是终点,必须照常调用工具,不要拿一段文字当答复。
- **字段值不因此变成模型的。** 模板里点名的画幅 / 背景该走 `aspect_ratio` / `background`,但**不要求从提示词正文里删掉** —— 原文不动优先,重复无害,删字才是损失。

`npm run smoke` 对这段有三条正向断言(**必须提到整段直传 / 必须要求照常调用 / 必须写明字段值另算**),外加两条反向断言:**「默认只填 `prompt`」与「面板参数的值不要写进 prompt」两段原文不得被稀释**,以及一条常驻成本上限(`tool description ≤ 3000` 字符)。
